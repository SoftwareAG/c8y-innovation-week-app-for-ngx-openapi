...
const ensureDir = (0, util_1.promisify)(fs_extra_1.ensureDir);
class LocaleUpdateCommand extends Commands_1.Commands {
    ...
    // TODO: clarify should _inputDir or this.inputDir be used?
    getExtractedPoFiles(_inputDir) {
        return (0, fast_glob_1.async)((0, path_1.join)(this.inputDir, '*.po'));
    }
    async action(cmd) {
        const root = process.cwd();
        const getExtractor = this.localeUtils.getExtractor.bind(this.localeUtils);
        this.inputDir = (0, path_1.resolve)(root, cmd.input || '.');
        const [modulePaths, pos] = await Promise.all([
            this.localeUtils.findModulePaths(root),
            this.localeUtils.getPoObjects([this.inputDir])
        ]);
        const modules = await Promise.all(modulePaths.map(path => getExtractor([path]).then(extractor => ({ extractor, path }))));
    ...
    async saveNewPos(newPos, modulePath) {
        ...
        _.forEach(newPos, newPo => {
            ...
            const folder = (0, path_1.resolve)(modulePath, 'locales');
            const file = (0, path_1.resolve)(folder, `${Language}.po`);
            const save = (0, util_1.promisify)(newPo.save.bind(newPo));
