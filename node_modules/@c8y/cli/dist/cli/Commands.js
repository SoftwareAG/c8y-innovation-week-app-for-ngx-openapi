...
class Commands {
    ...
    getWebpackExtraOptions() {
        return Object.assign({ hmr: this.getOption('hot') }, this.cli.options.env, this.cli.options.cli, {
    ...
    async getApps(folderGlobs) {
        ...
        for (const g of folderGlobs) {
            folderPaths = folderPaths.concat(await (0, fast_glob_1.async)(g, { onlyDirectories: true }));
        }
        for (const g of folderGlobs) {
            filePaths = filePaths.concat(await (0, fast_glob_1.async)(g, { onlyFiles: true, absolute: true }));
        }
        for (const p of filePaths) {
            if (/package.json$/.test(p)) {
                const app = await this.getPackageApp(p);
                if (app) {
                    apps.push(app);
                }
            }
    ...
    async getManifestApps(cwd, pattern = options_1.options['GLOB.LEGACY_APP_MANIFESTS']) {
        const manifestPaths = await (0, fast_glob_1.async)(pattern, { absolute: true, cwd });
        const apps = [];
    ...
    async getManifestApp(filePath) {
        ...
        const appOptions = this.parseOptions(this.cli.options.app);
    ...
    parseOptions(obj) {
        ...
        for (const key of Object.keys(obj)) {
            ...
            if (typeof value === 'string') {
                ...
                parsedOptions[key] = this.parseOptions(value);
                continue;
            }
    ...
    async getPackageApps(cwd) {
        const packagePaths = await (0, fast_glob_1.async)('package.json', { absolute: true, cwd });
        const apps = [];
    ...
    async getPackageApp(filePath) {
        ...
        return packageAppOptions
            ? new webpack_1.Application(Object.assign(Object.assign(Object.assign({}, allowedPkgJsonRootProps), packageAppOptions), this.parseOptions(this.cli.options.app)), entryPoint)
            : undefined;
    }
}
