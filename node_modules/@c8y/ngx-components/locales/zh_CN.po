...
msgstr "通过您的授权服务器管理的用户无法更改该用户的电子邮件。"
...
msgstr ""
"完成激活后，您将不再能够使用 <code>{{ vm.domainName }}</code> 访问租户\n"
"，请改用新的自定义域名 <code>{{ vm.newDomainName }}</code>。"
...
msgstr "应用程序输出创建因状态代码 {{ statusCode }} 而失败。"

msgid "Application output update failed due to status code {{ statusCode }}."
msgstr "应用程序输出更新因状态代码 {{ statusCode }} 而失败。"
...
msgstr "向 Actility 平台的验证失败，状态代码为 {{ statusCode }}。请检查配置文件 ID 是否正确。"
...
msgstr "向 LORIOT 平台的验证失败，状态代码为 {{ statusCode }}。请检查基本 URL、凭据是否正确，以及具有给定 ID 的应用程序是否存在。"
...
msgstr "未配置连接设置。在“管理”应用程序的<a href=\"{{ link }}\">设置</a>下方配置此类设置。"
...
msgstr "无法从 LoRa 平台获取连接计划。在“管理”应用程序的<a href=\"{{ link }}\">设置</a>下验证 ThingPark 凭据。"
...
msgstr "无法更新名称为 \"{{ connectionName }}\" 的 LNS 连接，因为它与 {{ numberOfDevices }} 个设备关联。"
...
msgstr "无法在具有已定义的固件、软件或配置的配置文件上更改设备类型，因为它们可能不适用于新设备类型的设备。"
...
msgstr "是否要将版本 {{version}} 设置为最新版本？部署新应用程序将始终使用此版本。"
...
msgstr "检索给定 ID 为 {{ applicationId }} 且状态代码为 400 的应用程序时出错。请检查所选应用程序是否正确"
...
msgstr "必须是有效的电话号码（仅允许使用数字、空格、正斜线 (\"/\")、短划线 (\"-\") 和加号 (\"+\")，例如：+49 9 876 543 210）。"
...
msgstr "占位符：{host}、{tenant-domain}、{username}"
...
msgstr "占位符：{tenant-domain}、{host}、{token}、{username}、{email}。例如，重置密码的整个链接可以是：{tenant-domain}/apps/devicemanagement/index.html?token={token}&email={email}"
...
msgstr "占位符：{tenant-domain}、{host}、{token}、{username}、{email}。例如，设置密码的整个链接可以是：{tenant-domain}/apps/devicemanagement/index.html?token={token}"
...
msgstr "在导航器中的位置（10000 在前，-10000 在后）。"
...
msgstr "提供 Actility ThingPark 连接设置。"
...
msgstr "至少应有 {{ requiredLength }} 个字符。"
...
msgstr "最大图像大小为 {{ maxFileSize }} kB。所选图像的大小为 {{ actualFileSize }} kB。"
...
msgstr "无法检索 ID 为 {{ markdownBinaryId }} 的二进制文件"
...
msgstr "您即将将此设备的所有者更改为 \"{{ owner }}\" 用户。除非相应地更改设备上的凭据，否则这可能会中断此设备与平台之间的通信。是否要继续？"
...
msgstr "您即将从仪表板中删除小组件 \"{{ widgetTitle }}\"。是否要继续？"
...
msgstr "您即将重置此仪表板。对仪表板的所有更改都将丢失，无法恢复。是否要继续？"
...
msgstr "您可以输入一组 LWM2M shell 命令。每个命令都将被发送到每个新注册的 LWM2M 设备。必须在新行中输入每个命令。使用以下语法指定注册后操作："

msgid "You can provide some examples displayed as placeholder…"
msgstr "您可以提供一些显示为占位符的示例…"
...
msgstr "您已达到 {{ versionsLimit }} 包版本的最大数量。要添加另一个版本，请先从列表中删除一个包。"
...
msgstr "例如 https://impact.example.cn"
