...
.make-el-increment(@range; @el; @delay: 0.05s; @lastIndex: 100; @pos: top; @size: 40px )
  when
  (@range > 0) {
  @{el}:nth-child(@{range}) {
    transition-delay: (@range * @delay);
    z-index: (@lastIndex - @range);
    @{pos}: -340px;
  }
  .make-el-increment((@range - 1), @el, @delay, @lastIndex, @pos, @size );
}
